...
| **R-06** | Reapplication: no open application (DB) **and** the newest `rejected` application by email/mobile is older than `reapplication_cooldown_days`. **A person/email that already has a `memberships` row cannot open a new application — they renew** (one member identity for life). | temporal + cross-row | eligibility check + Form Request |
...
| Reapplication after rejection | new `membership_applications` row after cooldown; earlier rows untouched. A person who already has a `memberships` row renews instead (R-06). |
