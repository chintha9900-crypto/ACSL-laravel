...
3. **[COMMERCIAL]** The actual priority/order values ACI wants once more than one promotion exists (the resolution *mechanism* is confirmed and designed; the concrete ordering of real promotions is data entry at launch, not a design question). — `04_MEMBERSHIP_ARCHITECTURE.md` §5.
...
6. **[COMMERCIAL]** Exact renewal pricing and renewal workflow/UX once a membership approaches or reaches expiry (the *rules that exist* — reminders, no auto-charge, no auto-convert — are confirmed; the actual renewal process is not). — `04_MEMBERSHIP_ARCHITECTURE.md` §9, `WORKFLOWS.md` §0.12.
